...
/// Returns the *Manhattan distance* (abs(drow) + abs(dcol)) between
/// **two node_ids**.
///
...
fn search_drainage(
    ...
    if start_accum >= threshold {
        ...
        let drainage = Drainage {
            node_id: start_index,
            closest: drainage_ids[0..num_to_store].to_vec(),
        };
        let mut drainage_guard = closest_drainage.lock().unwrap();
        drainage_guard[start_index as usize] = drainage;
    }
...
fn select_paths(paths: &Vec<Vec<u32>>, data: &Vec<RefCell<Node>>, alpha: f64) -> u32 {
    ...
    let path_lengths: HashMap<usize, f64> = paths
        .iter()
        .enumerate()
        .map(|(index, path)| (index, path.len() as f64))
        .collect();
    ...
    let mut filtered_paths: Vec<Vec<u32>> = paths
        .iter()
        .enumerate()
        .filter(|(index, _)| scores[*index] == max_score)
...
fn main() {
    ...
    let paths_to_drainage: Arc<Mutex<Vec<Vec<Vec<u32>>>>> =
        Arc::new(Mutex::new(vec![Vec::new(); data_len]));

    //arc mutex progress bar
    let task_count: Vec<u64> = closest_drainage
        .iter()
        .cloned()
        .map(|drain| drain.closest.len() as u64)
        .collect();
    ...
    match hand.write_npy(writer) {
        Ok(_) => {
            println!("{} was written.", output_hand_file)
        }
        Err(err) => {
            println!(
    ...
    match file.write_all(json_string.as_bytes()) {
        Ok(_) => {
            println!("{} was written.", output_neighbor_file)
        }
        Err(err) => {
            println!(
                "An error occured while writing {}. See below:\n{}",
